#!/usr/bin/env node
/**
 * claude-bridge-stream.mjs v2
 * 
 * Two Claude instances communicating via streaming JSON.
 * Filters out internal messages and handles duplicate events.
 * 
...
class ClaudeAgent {
    ...
    handleStreamEvent(event) {
        switch (event.type) {
            case 'start':
                // Debounce duplicate start events (within 200ms)
                const now = Date.now();
                if (now - this.lastStartTime < 200) {
            ...
            case 'text':
                // Detect internal JSON messages (topic classification, etc)
                const chunk = event.chunk || '';
                const full = event.full || '';
                
                // Check if this is internal JSON
                if (full.includes('isNewTopic') || 
                    full.includes('"title"') ||
                    chunk.includes('isNewTopic') ||
                    (full.trim().startsWith('{') && full.includes('"title"'))) {
                    this.isInternalMessage = true;
                    return; // Skip internal JSON
                }
                
                // Skip if we're in an internal message
                if (this.isInternalMessage) {
                    return;
                }
                
                // Accumulate the actual response
                this.currentResponse = full;
                break;
            ...
            case 'stop':
                ...
                if (this.currentResponse.trim()) {
                    log(this.name, `✓ Response: "${this.currentResponse.substring(0, 80)}${this.currentResponse.length > 80 ? '...' : ''}"`);
...
async function main() {
    console.log(`
╔══════════════════════════════════════════════════════════════╗
║         🤖 Claude Bridge (Streaming JSON v2)                 ║
║                                                              ║
║  • Filters internal JSON messages                            ║
║  • Dedupes start events                                      ║
