...
fn create_command_with_env(program: &str) -> Command {
    ...
    let mut tokio_cmd = if cfg!(target_os = "windows") && (program.ends_with(".cmd") || program.ends_with(".bat")) {
    ...
    for (key, value) in std::env::vars() {
        if key == "PATH"
            ...
            || key == "HOMEBREW_PREFIX"
            || key == "HOMEBREW_CELLAR"
            // Windows-specific environment variables for user profile and credentials
            || key == "USERPROFILE"
            || key == "APPDATA"
            || key == "LOCALAPPDATA"
            || key == "HOMEDRIVE"
...
fn create_system_command(claude_path: &str, args: Vec<String>, project_path: &str) -> Command {
    ...
    cmd.current_dir(project_path)
        .stdin(Stdio::null())        // Close stdin so Claude doesn't wait for input
        .stdout(Stdio::piped())
        .stderr(Stdio::piped());
...
pub async fn execute_claude_code(
    ...
    let claude_path = find_claude_binary(&app)?;
    log::info!("Using Claude binary at: {}", claude_path);
    ...
    let args = vec![
        ...
        prompt.clone(),  // Prompt is a positional argument, must come at the end
    ];
...
async fn spawn_claude_process(
    ...
    log::info!("About to spawn Claude process for prompt: {}", prompt);
    ...
    let stdout_task = tokio::spawn(async move {
        ...
        log::info!("Starting stdout reader task");
        while let Ok(Some(line)) = lines.next_line().await {
            log::info!("Claude stdout line received: {}", line.chars().take(100).collect::<String>());
    ...
    let stderr_task = tokio::spawn(async move {
        ...
        log::info!("Starting stderr reader task");
        while let Ok(Some(line)) = lines.next_line().await {
            log::error!("Claude stderr line received: {}", line);
