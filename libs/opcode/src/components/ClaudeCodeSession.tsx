...
export const ClaudeCodeSession: React.FC<ClaudeCodeSessionProps> = ({
  ...
  const handleSendPrompt = async (prompt: string, model: "sonnet" | "opus") => {
    ...
    try {
      ...
      // Set a timeout to reset loading state if no events are received
      ...
      loadingTimeoutRef.current = setTimeout(() => {
        if (isMountedRef.current && hasActiveSessionRef.current) {
          console.warn('[ClaudeCodeSession] Timeout: No completion event received after 5 minutes, resetting loading state');
          setIsLoading(false);
          hasActiveSessionRef.current = false;
          isListeningRef.current = false;
          setError("Session timed out - no response received. The process may still be running in the background.");
        }
      ...
      if (!isListeningRef.current) {
        ...
        const attachSessionSpecificListeners = async (sid: string) => {
          ...
          const specificErrorUnlisten = await listen(`claude-error:${sid}`, (evt: any) => {
            ...
            // On error, also clear the timeout and reset loading state
            if (loadingTimeoutRef.current) {
              clearTimeout(loadingTimeoutRef.current);
              loadingTimeoutRef.current = null;
            }
        ...
        const genericOutputUnlisten = await listen('claude-output', async (event: any) => {
          console.log('[ClaudeCodeSession] Received claude-output (generic):', event.payload?.substring?.(0, 100));
          ...
          try {
            ...
            console.warn('[ClaudeCodeSession] Failed to parse message for session ID extraction:', parseErr);
          }
        });
        ...
        const processComplete = async (success: boolean) => {
          // Clear the timeout since we received a completion event
          if (loadingTimeoutRef.current) {
            clearTimeout(loadingTimeoutRef.current);
            loadingTimeoutRef.current = null;
          }
        ...
        if (effectiveSession && !isFirstPrompt) {
          ...
          try {
            ...
            console.log('[ClaudeCodeSession] resumeClaudeCode command completed');
          } catch (cmdErr) {
            console.error('[ClaudeCodeSession] resumeClaudeCode command failed:', cmdErr);
            throw cmdErr; // Re-throw to be caught by outer catch
          }
        } else {
          ...
          try {
            ...
            console.log('[ClaudeCodeSession] executeClaudeCode command completed');
          } catch (cmdErr) {
            console.error('[ClaudeCodeSession] executeClaudeCode command failed:', cmdErr);
            throw cmdErr; // Re-throw to be caught by outer catch
          }
        }
      }
