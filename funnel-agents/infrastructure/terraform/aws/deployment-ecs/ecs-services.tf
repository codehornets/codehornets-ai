...
resource "aws_ecs_task_definition" "app_modules" {
  ...
  cpu                      = lookup(var.module_configs, each.key, {}).cpu != null ? var.module_configs[each.key].cpu : var.ecs_cpu
  memory                   = lookup(var.module_configs, each.key, {}).memory != null ? var.module_configs[each.key].memory : var.ecs_memory
...
resource "aws_ecs_service" "app_modules" {
  ...
  desired_count   = lookup(var.module_configs, each.key, {}).desired_count != null ? var.module_configs[each.key].desired_count : var.ecs_desired_count
...
resource "aws_appautoscaling_target" "app_modules" {
  ...
  max_capacity       = lookup(var.module_configs, each.key, {}).max_capacity != null ? var.module_configs[each.key].max_capacity : var.ecs_max_capacity
  min_capacity       = lookup(var.module_configs, each.key, {}).min_capacity != null ? var.module_configs[each.key].min_capacity : var.ecs_min_capacity
