 ...
 */
export function escapeSpecialChars(content) {
    return content
 ...
 */
export function unescapeSpecialChars(content) {
    return content
        .replaceAll(AMPERSAND_TOKEN, '&')
        .replaceAll(DOLLAR_TOKEN, '$');
}
...
export function createDiffHunks(options) {
    const { filePath, oldContent, newContent, ignoreWhitespace = false, singleHunk = false, } = options;
    ...
    const rawHunks = generateUnifiedDiff(escapedOld, escapedNew, contextLines, ignoreWhitespace);
 ...
 * ```
 */
export function createDiffFromEdits(options) {
    ...
    const rawHunks = generateUnifiedDiff(escapedOriginal, escapedNew, DEFAULT_CONTEXT_LINES, ignoreWhitespace);
    // Unescape tokens in the result
    return rawHunks.map((hunk) => ({
        ...hunk,
        lines: hunk.lines.map(unescapeSpecialChars),
 ...
 * @returns Array of diff hunks
 */
function generateUnifiedDiff(oldContent, newContent, contextLines, ignoreWhitespace) {
 ...
 * @returns LCS table
 */
function computeLCS(oldLines, newLines, ignoreWhitespace) {
 ...
 */
function generateDiffOperations(oldLines, newLines, lcs) {
    const operations = [];
 ...
 * @returns Array of diff hunks
 */
function groupIntoHunks(oldLines, newLines, operations, contextLines) {
 ...
 */
function createHunk(oldLines, newLines, operations, start, end) {
    const lines = [];
    let oldStart = -1;
    let newStart = -1;
    let oldCount = 0;
    ...
    for (let i = start; i <= end; i++) {
        ...
        switch (op.type) {
            case 'keep':
                ...
                lines.push(` ${oldLines[op.oldIndex]}`);
                oldCount++;
                newCount++;
                break;
            case 'delete':
            ...
            case 'insert':
                ...
                lines.push(`+${newLines[op.newIndex]}`);
                newCount++;
                break;
        }
    }
 ...
 */
export function calculateLineStats(hunks, newContent) {
    ...
    const linesAdded = hunks.reduce((sum, hunk) => sum + hunk.lines.filter((line) => line.startsWith('+')).length, 0);
    const linesRemoved = hunks.reduce((sum, hunk) => sum + hunk.lines.filter((line) => line.startsWith('-')).length, 0);
