 ...
 */
export function getNamedMeter(name, version) {
...
export class HistogramRecorder {
     ...
     */
    async time(operation, attributes) {
...
class Buckets {
    ...
    indexStart;
    indexEnd;
    constructor(backing = [0], indexBase = 0, indexStart = 0, indexEnd = 0) {
    ...
    downscale(amount) {
        ...
        // Merge buckets
        const newBacking = new Array(newEnd - newStart + 1).fill(0);
...
export class ExponentialHistogramAccumulation {
     ...
     */
    clone() {
        const clone = new ExponentialHistogramAccumulation({ name: "", maxSize: this.maxSize, recordMinMax: this.recordMinMax, maxScale: this.scale }, this.startTime);
        clone.sum = this.sum;
 ...
 */
export function createUpDownCounter(config) {
    const meter = getMeter();
 ...
 */
export function createGauge(config, callback) {
    const meter = getMeter();
