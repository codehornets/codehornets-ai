 ...
 */
export function getNamedTracer(name, version) {
 ...
 */
export async function withSpanAsync(name, fn, options) {
    const tracer = getTracer();
    return tracer.startActiveSpan(name, {
        kind: options?.kind ?? SpanKind.INTERNAL,
        attributes: options?.attributes ? sanitizeAttributes(options.attributes) : undefined,
 ...
 */
export function recordException(spanOrError, errorOrEscaped, escaped = true) {
    let span;
    let error;
    let isEscaped = escaped;
    // Handle overloaded arguments
    if (spanOrError instanceof Error || !spanOrError.recordException) {
 ...
 * @returns The created span
 */
export function createDetachedSpan(name, options) {
    const tracer = getTracer();
    return tracer.startSpan(name, {
        kind: options?.kind ?? SpanKind.INTERNAL,
        attributes: options?.attributes ? sanitizeAttributes(options.attributes) : undefined,
 ...
 */
export function buildSpan(name) {
 ...
 */
export function traceCommand(command, fn, attributes) {
 ...
 */
export async function traceApiCall(endpoint, method, fn) {
 ...
 */
export function traceTool(toolName, fn, attributes) {
