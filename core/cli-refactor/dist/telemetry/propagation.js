 ...
 */
export function encodeBaggageValue(value) {
    // Encode characters that are not allowed in baggage values
    return encodeURIComponent(value).replace(/[-_.!~*'()]/g, (c) => `%${c.charCodeAt(0).toString(16).toUpperCase()}`);
}
/**
 ...
 */
export function decodeBaggageValue(value) {
    try {
 ...
 */
export function serializeBaggageEntry(key, entry) {
 ...
 */
export function getKeyPairs(entries) {
    const pairs = [];
 ...
 */
export function serializeKeyPairs(pairs) {
 ...
 */
export function parsePairKeyValue(pair) {
 ...
 */
export function parseKeyPairsIntoRecord(headerValue) {
    const result = {};
    if (!headerValue)
        return result;
...
export class W3CBaggagePropagator {
    ...
    extract(context, carrier, getter) {
        ...
        const baggage = propagation.createBaggage(baggageEntries.reduce((acc, { key, value, metadata }) => {
            acc[key] = { value, metadata: metadata };
            return acc;
        }, {}));
 ...
 */
export function getBaggageValue(context, key) {
    const baggage = propagation.getBaggage(context);
 ...
 */
export function setBaggageValue(context, key, value, metadata) {
 ...
 */
export function removeBaggageValue(context, key) {
    const currentBaggage = propagation.getBaggage(context);
    if (!currentBaggage)
        return context;
 ...
 */
export function getAllBaggage(context) {
    const baggage = propagation.getBaggage(context);
    if (!baggage)
        return {};
    const result = {};
