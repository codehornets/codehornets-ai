...
export class WslPathConverter {
    ...
    toLocalPath(windowsPath) {
        ...
        if (this.wslDistroName) {
            const wslMatch = windowsPath.match(/^\\\\wsl(?:\.localhost|\$)\\([^\\]+)(.*)$/);
        ...
        try {
            ...
            return windowsPath
                .replace(/\\/g, '/')
                .replace(/^([A-Z]):/i, (_match, driveLetter) => {
 ...
 * ```
 */
export function isPathForDistro(path, distroName) {
 ...
 * ```
 */
export function getWindowsUserProfile() {
