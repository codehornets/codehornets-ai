 ...
 */
export function findVsCodeCli(ideType) {
            ...
            if (cmdOutput) {
                ...
                for (const [appName, cliName] of Object.entries(appMappings)) {
                    ...
                    if (index !== -1) {
                        ...
                        return (cmdOutput.substring(0, bundleEnd) +
 ...
 */
export async function getInstalledVsCodeExtensionVersion(cliPath) {
    const result = await executeCommand(cliPath, ['--list-extensions', '--show-versions'], { env: getVsCodeEnv() });
 ...
 * ```
 */
export async function isExtensionInstalled(ideType) {
    if (isVsCodeIde(ideType)) {
        const cliPath = findVsCodeCli(ideType);
        if (cliPath) {
            try {
                const result = await executeCommand(cliPath, ['--list-extensions'], { env: getVsCodeEnv() });
 ...
 */
function isJetBrainsPluginInstalled(_ideType) {
 ...
 */
async function installVsCodeExtension(cliPath) {
    // Wait a bit before installing (allows VS Code to settle)
    await new Promise((resolve) => setTimeout(resolve, 500));
    const result = await executeCommand(cliPath, ['--force', '--install-extension', VSCODE_EXTENSION_ID], { env: getVsCodeEnv() });
    if (result.code !== 0) {
 ...
 */
async function installJetBrainsPlugin(_ideType, _pluginPath) {
    ...
    try {
        ...
        if (isVsCodeIde(ideType)) {
            ...
            if (platform !== 'wsl') {
                ...
                const pluginPath = resolve(moduleDir, 'vendor', 'claude-code-jetbrains-plugin');
 ...
 */
export function isWindsurfCliAvailable() {
    try {
