...
export class DiagnosticsManager {
     ...
     */
    initialize(client, mcpRequestFn) {
        if (this.initialized) {
            return;
        }
    ...
    async ensureFileOpened(filePath) {
        ...
        try {
            await this.mcpRequest('openFile', {
                filePath,
                preview: false,
                startText: '',
                endText: '',
    ...
    async beforeFileEdited(filePath) {
        ...
        try {
            const result = await this.mcpRequest('getDiagnostics', { uri: `file://${filePath}` }, this.mcpClient);
            const diagnostics = this.parseDiagnosticResult(result)[0];
            if (diagnostics) {
                if (filePath !== this.normalizeFileUri(diagnostics.uri)) {
                    throw new DiagnosticsError(`Diagnostics file path mismatch: expected ${filePath}, got ${diagnostics.uri}`);
                }
    ...
    async getNewDiagnostics() {
        ...
        let allDiagnostics = [];
        try {
            const result = await this.mcpRequest('getDiagnostics', {}, this.mcpClient);
        ...
        for (const fileDiag of fileDiagnostics) {
            ...
            if (rightFileDiag) {
                ...
                this.rightFileDiagnosticsState.set(normalizedUri, rightFileDiag.diagnostics);
            }
            // Find diagnostics not in the baseline
            const newItems = currentDiag.diagnostics.filter((d) => !baseline.some((b) => this.areDiagnosticsEqual(d, b)));
    ...
    parseDiagnosticResult(result) {
        if (Array.isArray(result)) {
            const textContent = result.find((item) => typeof item === 'object' &&
                item !== null &&
                'type' in item &&
                item.type === 'text');
     ...
     */
    async handleQueryStart(connectionStates) {
        if (!this.initialized) {
