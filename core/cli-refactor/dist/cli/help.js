 ...
 */
export function generateMainHelp() {
    const lines = [];
 ...
 * @returns Formatted help string
 */
export function generateMCPHelp() {
    const lines = [];
    ...
    for (const entry of mcpCliCommands) {
        const cmd = entry[0] ?? "";
        const desc = entry[1] ?? "";
        lines.push(`  ${formatCommand(padEnd(cmd, maxCmdLen + 2))} ${desc}`);
    }
    ...
    lines.push('  claude mcp add --transport stdio airtable --env AIRTABLE_API_KEY=YOUR_KEY -- npx -y airtable-mcp-server');
    lines.push('');
    lines.push(formatHeader('Options:'));
    const options = [
 ...
 * @returns Formatted help string
 */
export function generatePluginHelp() {
    const lines = [];
    ...
    for (const [cmd, desc] of marketplaceCommands) {
        if (!cmd)
            continue;
        lines.push(`  ${formatCommand(padEnd(cmd, maxCmdLen + 2))} ${desc}`);
    }
 ...
 * @returns Formatted help string
 */
export function generateDoctorHelp() {
    const lines = [];
 ...
 * @returns Formatted help string
 */
export function generateUpdateHelp() {
    const lines = [];
 ...
 */
export function generateVersionText() {
