...
function displayUpdateResult(result) {
    ...
    if (result.updateAvailable) {
        output.success(`${SYMBOLS.tick} Update available: ${result.currentVersion} ${SYMBOLS.arrow} ${result.latestVersion}`);
 ...
 * @returns Exit code
 */
export async function handleUpdateCommand(args, options, context) {
    if (options.help) {
 ...
 * @returns Exit code
 */
export async function handleInstallCommand(args, options, context) {
    const [target] = args;
 ...
 * @returns Exit code
 */
export async function handleMigrateInstallerCommand(args, options, context) {
    if (isLocalInstallation()) {
 ...
 * @returns Exit code
 */
export async function handleSetupTokenCommand(args, options, context) {
    ...
    const hasExistingAuth = !!process.env.ANTHROPIC_API_KEY ||
