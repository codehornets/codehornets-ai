...
import { validateScope, validateTransport, parseEnvArgs, parseHeaderArgs, parseToolIdentifier, parseJSON, looksLikeUrl, } from '../parser.js';
...
async function readMCPState() {
    ...
    throw new Error('MCP state not available. The mcp command is only available within a Claude Code session.');
}
/**
 ...
 * @returns Exit code
 */
export async function handleMCPServe(args, options, context) {
 ...
 * @returns Exit code
 */
export async function handleMCPAdd(args, options, context) {
    ...
    try {
        ...
        if (transport === 'sse') {
            ...
            if (!transportExplicit && urlLike) {
                output.warn('');
                output.warn(`Warning: The command "${commandOrUrl}" looks like a URL, but is being interpreted as a stdio server as --transport was not specified.`);
            ...
            output.success(`Added stdio MCP server ${name} with command: ${commandOrUrl} ${extraArgs.join(' ')} to ${scope} config`);
        }
 ...
 * @returns Exit code
 */
export async function handleMCPRemove(args, options, context) {
    const [name] = args;
    if (!name) {
        output.error('Error: Server name is required.');
        return ExitCodes.INVALID_ARGS;
 ...
 * @returns Exit code
 */
export async function handleMCPList(args, options, context) {
 ...
 * @returns Exit code
 */
export async function handleMCPGet(args, options, context) {
    const [name] = args;
    if (!name) {
        output.error('Error: Server name is required.');
        return ExitCodes.INVALID_ARGS;
 ...
 * @returns Exit code
 */
export async function handleMCPAddJSON(args, options, context) {
    ...
    try {
        const scope = options.scope ? validateScope(options.scope) : 'local';
        const config = parseJSON(jsonStr, 'MCP configuration');
 ...
 * @returns Exit code
 */
export async function handleMCPResetChoices(args, options, context) {
 ...
 * @returns Exit code
 */
export async function handleMCPServers(options) {
    try {
 ...
 * @returns Exit code
 */
export async function handleMCPTools(serverFilter, options) {
    try {
        const state = await readMCPState();
 ...
 * @returns Exit code
 */
export async function handleMCPInfo(toolId, options) {
    try {
 ...
 * @returns Exit code
 */
export async function handleMCPCall(toolId, argsJson, options) {
    try {
        const { server, tool } = parseToolIdentifier(toolId);
        let inputArgs;
        // Handle stdin input
 ...
 * @returns Exit code
 */
export async function handleMCPGrep(pattern, options) {
    try {
        let regex;
        try {
 ...
 * @returns Exit code
 */
export async function handleMCPResources(serverFilter, options) {
    try {
        const state = await readMCPState();
        let resources = [];
        if (serverFilter) {
 ...
 * @returns Exit code
 */
export async function handleMCPRead(resourceId, uri, options) {
    try {
        let server;
        let resourceName;
        let resourceUri;
 ...
 * @returns Exit code
 */
export async function handleMCPCommand(args, options, context) {
    ...
    switch (subcommand) {
        case 'serve':
            return handleMCPServe(subArgs, options, context);
        case 'add':
        ...
        case 'call':
            ...
            return handleMCPCall(subArgs[0], subArgs[1], options);
        case 'grep':
            if (!subArgs[0]) {
        ...
        case 'read':
            ...
            return handleMCPRead(subArgs[0], subArgs[1], options);
        default:
