 ...
 */
function createOptionMap(specs) {
    const map = new Map();
 ...
 * ```
 */
export function parseArgs(argv, additionalOptions = []) {
    ...
    while (i < argv.length) {
        ...
        if (arg && arg.startsWith('--') && arg.includes('=')) {
            ...
            if (spec && spec.property) {
                if (spec.multiple) {
                    const existing = options[spec.property];
...
export function validateTransport(transport) {
    ...
    throw new Error(`Invalid transport "${transport}". Valid options: ${validTransports.join(', ')}`);
}
/**
 ...
 * @throws Error if format is invalid
 */
export function validateInputFormat(format) {
...
export function validatePermissionMode(mode) {
    const validModes = ['default', 'bypassPermissions', 'plan'];
 ...
 * ```
 */
export function parseToolIdentifier(identifier) {
 ...
 */
export function parseJSON(jsonString, context = 'JSON') {
    try {
 ...
 */
export function extractSessionId(resume) {
