 ...
 */
import { fromEnv as sdkFromEnv, fromIni as sdkFromIni, fromProcess as sdkFromProcess, fromTokenFile as sdkFromTokenFile, fromContainerMetadata as sdkFromContainerMetadata, fromInstanceMetadata as sdkFromInstanceMetadata, fromSSO as sdkFromSSO, } from '@aws-sdk/credential-providers';
...
function chain(...providers) {
    return async () => {
        ...
        throw new CredentialsProviderError(`Could not load credentials from any providers. Errors: ${errors.map((e) => e.message).join(', ')}`, { tryNextLink: false });
    };
}
/**
...
export function fromEnv(options = {}) {
    return async (_init) => {
        ...
        const credentials = (await sdkFromEnv()());
 ...
 * ```
 */
export function fromIni(options = {}) {
    return async (init) => {
        const mergedOptions = {
            ...options,
            parentClientConfig: {
        ...
        const credentials = (await sdkFromIni(iniOptions)());
 ...
 * ```
 */
export function fromProcess(options = {}) {
    return async (init) => {
 ...
 * ```
 */
export function fromTokenFile(options = {}) {
    return async (_init) => {
        ...
        const credentials = (await sdkFromTokenFile(tokenOptions)());
 ...
 * ```
 */
export function fromContainerMetadata(options = {}) {
    return async () => {
 ...
 * ```
 */
export function fromInstanceMetadata(options = {}) {
    return async () => {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const credentials = (await sdkFromInstanceMetadata(imdsOptions)());
        return credentials;
    };
}
/**
 ...
 * ```
 */
export function fromSSO(options = {}) {
    return async (_init) => {
 ...
 * @returns Credential provider function
 */
export async function remoteProvider(options = {}) {
    // Check for container credentials
    if (process.env[ENV_CMDS_RELATIVE_URI] || process.env[ENV_CMDS_FULL_URI]) {
        options.logger?.debug('@aws-sdk/credential-provider-node - remoteProvider::fromContainerMetadata');
        return chain(fromContainerMetadata(options), fromContainerMetadata(options));
    }
    // Check if EC2 metadata is disabled
    if (process.env[ENV_EC2_METADATA_DISABLED] &&
    ...
    options.logger?.debug('@aws-sdk/credential-provider-node - remoteProvider::fromInstanceMetadata');
 ...
 * ```
 */
export function defaultProvider(options = {}) {
        ...
        if (options.profile ?? process.env[ENV_PROFILE]) {
            ...
            if (process.env[ENV_KEY] && process.env[ENV_SECRET]) {
                if (!hasLoggedProfileWarning) {
                    const warnFn = options.logger?.warn && options.logger?.constructor?.name !== 'NoOpLogger'
...
`);
    ...
    async () => {
        ...
        if (!ssoStartUrl && !ssoAccountId && !ssoRegion && !ssoRoleName && !ssoSession) {
            throw new CredentialsProviderError('Skipping SSO provider in default chain (inputs do not include SSO fields).', { logger: options.logger });
        }
 ...
 */
export function credentialsWillNeedRefresh(credentials) {
 ...
 */
export function credentialsTreatedAsExpired(credentials) {
    return (credentials?.expiration !== undefined &&
 ...
 */
export function decorateDefaultCredentialProvider(credentialProvider) {
    return (options) => {
