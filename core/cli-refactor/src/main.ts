...
import { runAndExit, run, ExitCodes, VERSION } from './cli/index.js';
 ...
 */
function isRipgrepMode(): boolean {
 ...
 * @returns Exit code
 */
async function handleMCPCLIMode(): Promise<number> {
 ...
 * @returns Exit code
 */
async function handleRipgrepMode(): Promise<number> {
  const args = process.argv.slice(3);
 ...
 */
function configureCorepack(): void {
