 ...
 */
export const IDE_CONFIGS: IdeConfigMap = {
  cursor: {
    ideKind: 'vscode',
    displayName: 'Cursor',
 ...
 * ```
 */
export function isVsCodeIde(ideType: IdeType | string | null): boolean {
  if (!ideType) {
    return false;
  }
  const config = IDE_CONFIGS[ideType as IdeType];
 ...
 * ```
 */
export function isJetBrainsIde(ideType: IdeType | string | null): boolean {
  if (!ideType) {
    return false;
  }
  const config = IDE_CONFIGS[ideType as IdeType];
 ...
 * ```
 */
export function getIdeDisplayName(ideType: IdeType | string | null): string {
  if (!ideType) {
    return 'IDE';
  }
  const config = IDE_CONFIGS[ideType as IdeType];
 ...
 */
export function getIdeConfig(ideType: IdeType | string): IdeConfig | undefined {
  return IDE_CONFIGS[ideType as IdeType];
}
 ...
 * ```
 */
export function detectRunningIdes(): IdeType[] {
  const detectedIdes: IdeType[] = [];
      ...
      for (const [ideType, config] of Object.entries(IDE_CONFIGS)) {
        for (const keyword of config.processKeywordsMac) {
          if (processOutput.includes(keyword)) {
            detectedIdes.push(ideType as IdeType);
            break;
          }
        }
      }
 ...
 */
export function isAncestorProcess(pid: number): boolean {
 ...
 */
export function isRunningInIdeTerminal(terminalType?: string | null): boolean {
 ...
 */
export function getIdeTerminalType(): IdeType | null {
  const platform = getPlatform();
  if (platform !== 'windows') {
    return null;
  }
