 ...
 */
let connectionWaitAbortController: AbortController | null = null;
  ...
  try {
    ...
    for (const lockFileInfo of lockFiles) {
      ...
      if (process.env.CLAUDE_CODE_IDE_SKIP_VALID_CHECK === 'true') {
        ...
        isValid = parsed.workspaceFolders.some((folder) => {
          ...
          if (platform === 'windows') {
            ...
            return (
              cwdNormalized === resolvedNormalized ||
      ...
      const terminalType = inIdeTerminal
        ? (process.env.TERM_PROGRAM as IdeType | undefined)
        : undefined;
      const ideName =
        parsed.ideName ?? (inIdeTerminal ? getIdeDisplayName(terminalType ?? null) : 'IDE');
    ...
    if (!includeInvalid && ssePort) {
      const portMatch = connections.filter(
        (conn) => conn.isValid && conn.port === ssePort
      );
 ...
 * ```
 */
export async function waitForIdeConnection(
  timeoutMs = 30000,
  pollIntervalMs = 1000
 ...
 */
export function cancelConnectionWait(): void {
  if (connectionWaitAbortController) {
    connectionWaitAbortController.abort();
 ...
 * ```
 */
export function hasIdeConnection(connectionStates: ConnectionState[]): boolean {
  return connectionStates.some(
 ...
 */
export function findIdeConnection(
  connectionStates: ConnectionState[] | undefined
): (ConnectionState & { type: 'connected' }) | undefined {
  if (!connectionStates) {
    return undefined;
  }
  ...
  return ideConnection?.type === 'connected'
    ? (ideConnection as ConnectionState & { type: 'connected' })
    : undefined;
}
 ...
 */
export function getIdeNameFromConnection(
  connectionState: McpClientState | undefined
): string | null {
  ...
  if (isRunningInIdeTerminal()) {
    const terminalType = process.env.TERM_PROGRAM as IdeType | undefined;
 ...
 * @returns The IDE name or null
 */
export function getIdeNameFromConnections(
