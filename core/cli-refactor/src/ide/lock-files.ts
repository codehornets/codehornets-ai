...
import type { LockFileInfo, ParsedLockFile, Platform } from './types.js';
 ...
 */
export function getLockFileDirectories(): string[] {
 ...
 * ```
 */
export function getIdeLockFiles(): LockFileInfo[] {
  const filesystem = getFs();
 ...
 */
export async function getIdeHost(
 ...
 * ```
 */
export async function cleanStaleLockFiles(): Promise<void> {
  const filesystem = getFs();
  const platform = getPlatform();
