 ...
 */
function isBasic(codePoint: number): boolean {
 ...
 */
function hasNonAscii(label: string): boolean {
 ...
 */
export function toASCII(domain: string, options: IdnaOptions = {}): string | null {
  const {
 ...
 */
export function toUnicode(domain: string, options: IdnaOptions = {}): IdnaToUnicodeResult {
  try {
    // Split into labels
    const labels = domain.split('.');
 ...
 */
export function urlToASCII(url: string): string {
  try {
