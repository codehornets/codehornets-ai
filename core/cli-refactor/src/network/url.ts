 ...
 */
const DEFAULT_PORTS: Record<string, number | null> = {
  ftp: 21,
  file: null,
  http: 80,
  https: 443,
...
export function parseUrl(input: string, base?: string): ParsedUrl {
  try {
    ...
    throw new UrlParseError(input, `Failed to parse URL: ${(error as Error).message}`);
  }
}
 ...
 */
export function tryParseUrl(input: string, base?: string): ParsedUrl | null {
  try {
 ...
 */
export function serializeOrigin(url: URL | ParsedUrl): string {
  if (url instanceof URL) {
    return url.origin;
  }
 ...
 */
export function serializePath(url: URL | ParsedUrl): string {
  if (url instanceof URL) {
    return url.pathname;
  }
  return url.pathname;
 ...
 */
export function getEffectivePort(url: URL | ParsedUrl): number | null {
 ...
 */
export function hasCredentials(url: URL | ParsedUrl): boolean {
 ...
 */
export function cannotHaveUsernamePasswordPort(url: URL | ParsedUrl): boolean {
 ...
 */
export function setUsername(url: URL, username: string): void {
 ...
 */
export function setPassword(url: URL, password: string): void {
 ...
 */
export function formatIPv6(hostname: string): string {
 ...
 */
export function getPathExtension(url: string | URL): string {
  const urlObj = typeof url === 'string' ? new URL(url) : url;
 ...
 */
export function isSecure(url: string | URL): boolean {
  const protocol =
    typeof url === 'string' ? tryParseUrl(url)?.protocol : url.protocol;
...
function decodePunycode(input: string): string {
  ...
  while (index < input.length) {
    ...
    for (let k = base; ; k += base) {
      ...
      if (value >= base) throw new Error('Invalid punycode');
