...
function isValidArray(array: unknown[]): boolean {
  ...
  for (const item of array) {
    if (item == null) continue; // null/undefined allowed in arrays
...
export function validateAttribute(
  ...
  if (Array.isArray(value)) {
    ...
    return { valid: true, value: sanitized as AttributeValue };
  }
...
export function sanitizeAttributes(
  ...
  for (const [key, value] of entries) {
    ...
    const validation = validateAttribute(key, value as AttributeValueType);
    ...
    if (Array.isArray(validation.value)) {
      result[key] = [...validation.value] as AttributeValue;
    } else {
