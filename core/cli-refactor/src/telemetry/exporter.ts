...
export const ExportResultCode = {
  SUCCESS: 0 as ExportResult,
  FAILED: 1 as ExportResult,
} as const;

/**
 * Creates a successful export result.
 */
export function createSuccessResult(): ExportResult {
 ...
 */
export function createFailedResult(): ExportResult {
 ...
 */
export function attributesToOTLP(attributes: Attributes): Array<{
  key: string;
  value: { stringValue?: string; intValue?: number; boolValue?: boolean; arrayValue?: unknown };
}> {
 ...
 */
export function spanToOTLP(span: SpanData): Record<string, unknown> {
  return {
    ...
    events: span.events.map((event) => ({
      ...
      name: event.name,
      attributes: event.attributes ? attributesToOTLP(event.attributes as Attributes) : [],
 ...
 */
export function createTraceExportRequest(batch: SpanExportBatch): Record<string, unknown> {
  return {
    resourceSpans: [
      {
        resource: {
 ...
 */
export function createMetricsExportRequest(batch: MetricExportBatch): Record<string, unknown> {
  return {
    resourceMetrics: [
      {
        resource: {
 ...
 */
export abstract class OTLPExporter<T> {
  protected readonly config: OTLPExporterConfig;
  protected readonly batchConfig: BatchExportConfig;
  protected queue: T[] = [];
  protected exportTimer: ReturnType<typeof setTimeout> | null = null;
   ...
   */
  protected async sendRequest(body: string): Promise<Response> {
    const controller = new AbortController();
    const timeout = setTimeout(
      () => controller.abort(),
      this.batchConfig.exportTimeoutMs
    );
 ...
 */
export class OTLPTraceExporter extends OTLPExporter<SpanData> {
  ...
  constructor(
    ...
    batchConfig?: Partial<BatchExportConfig>
  ) {
  ...
  protected async doExport(spans: SpanData[]): Promise<void> {
 ...
 */
export class OTLPMetricsExporter extends OTLPExporter<Record<string, unknown>> {
  private resource: Resource;
  private scope: InstrumentationScope;
   ...
   * Exports metric data.
   */
  export(metrics: Array<{ name: string; description?: string; unit?: string; data: unknown }>): void {
    this.enqueue(metrics);
  }

  protected async doExport(metrics: Record<string, unknown>[]): Promise<void> {
    const request = createMetricsExportRequest({
      resource: this.resource,
      scope: this.scope,
      metrics: metrics as MetricExportBatch['metrics'],
    });
...
export class ConsoleSpanExporter {
  ...
  export(spans: SpanData[]): void {
    for (const span of spans) {
      ...
      console.log(
        `[TRACE] ${span.name} (${duration.toFixed(2)}ms)`,
        this.includeDetails
          ? {
...
export function createTraceExporter(
  config: OTLPExporterConfig | null,
  resource: Resource,
  scope: InstrumentationScope,
  ...
  return new NoOpExporter<SpanData>();
}
...
export function createMetricsExporter(
  ...
  resource: Resource,
  scope: InstrumentationScope
): OTLPMetricsExporter | NoOpExporter<Record<string, unknown>> {
  if (config) {
    return new OTLPMetricsExporter(config, resource, scope);
  }

  return new NoOpExporter<Record<string, unknown>>();
}
