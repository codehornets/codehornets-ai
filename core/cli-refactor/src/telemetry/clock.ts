 ...
 */
export const defaultWallClock: Clock = {
  now(): number {
    return Date.now();
  },
};
...
export class AnchoredClock implements Clock {
  private readonly _monotonicClock: MonotonicClock;
  private readonly _epochMillis: number;
  private readonly _performanceMillis: number;
 ...
 */
let sharedClock: AnchoredClock | null = null;
 ...
 */
export function getSharedClock(): AnchoredClock {
  if (!sharedClock) {
