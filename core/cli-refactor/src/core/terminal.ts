 ...
 */
function getForcedColorLevel(): ColorLevel | undefined {
  ...
  if (forceColor === 'true') return ColorLevel.Basic;
  if (forceColor === 'false') return ColorLevel.None;

  if (forceColor.length === 0) return ColorLevel.Basic;

  return Math.min(parseInt(forceColor, 10), 3) as ColorLevel;
}
...
export function detectColorLevel(
  ...
  if ('CI' in env) {
    if (
      ['GITHUB_ACTIONS', 'GITEA_ACTIONS', 'CIRCLECI'].some((ci) => ci in env)
    ) {
      return ColorLevel.TrueColor;
    }
    if (
      ['TRAVIS', 'APPVEYOR', 'GITLAB_CI', 'BUILDKITE', 'DRONE'].some((ci) => ci in env) ||
  ...
  if (env.COLORTERM === 'truecolor') return ColorLevel.TrueColor;
  if (env.TERM === 'xterm-kitty') return ColorLevel.TrueColor;
...
let stdoutSupport: ColorSupport | false | undefined;
let stderrSupport: ColorSupport | false | undefined;

/**
 * Gets color support for stdout
 * @returns Color support object or false
 */
export function getStdoutColorSupport(): ColorSupport | false {
 ...
 * @returns Color support object or false
 */
export function getStderrColorSupport(): ColorSupport | false {
 ...
 * @returns Styled text
 */
function wrap(text: string, openCode: number, closeCode: number): string {
 ...
 */
function createStyleFunction(styleName: AnsiStyleName): (text: string) => string {
  const [open, close] = ANSI_CODES[styleName];
  return (text: string) => wrap(text, open, close);
}
 ...
 * @returns Function that applies the background color to text
 */
export function bgHex(hexColor: string): (text: string) => string {
  const [r, g, b] = hexToRgb(hexColor);
  return bgRgb(r, g, b);
}
 ...
 * @returns Function that applies the color to text
 */
export function ansi256(code: number): (text: string) => string {
  return (text: string) => `${CSI}38;5;${code}m${text}${CSI}39m`;
}
 ...
 * @returns Function that applies the background color to text
 */
export function bgAnsi256(code: number): (text: string) => string {
  return (text: string) => `${CSI}48;5;${code}m${text}${CSI}49m`;
}
...
export function truncate(text: string, maxLength: number, ellipsis = '...'): string {
  ...
  if (visible <= maxLength) return text;
 ...
 * @returns ANSI sequence
 */
export function clearScreenDown(): string {
  return `${CSI}J`;
}
 ...
 * @returns ANSI sequence
 */
export function clearScreenUp(): string {
  return `${CSI}1J`;
}
 ...
 * @returns ANSI sequence
 */
export function clearLineEnd(): string {
  return `${CSI}K`;
}
 ...
 * @returns ANSI sequence
 */
export function clearLineStart(): string {
  return `${CSI}1K`;
}
