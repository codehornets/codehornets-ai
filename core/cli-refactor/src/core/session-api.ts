 ...
 */
export async function getOrganizationUuid(accessToken: string): Promise<string | null> {
  ...
  try {
    ...
    const data = await response.json() as { organization?: { uuid?: string } };
 ...
 */
export function buildSessionHeaders(accessToken: string, orgUuid: string): Record<string, string> {
  return {
    Authorization: `Bearer ${accessToken}`,
    'Content-Type': 'application/json',
...
export async function getEnvironments(
  ...
  try {
    ...
    const data = await response.json() as { data?: Environment[] };
    return data.data ?? [];
  } catch (error) {
...
export async function getSessionEvents(
  ...
  try {
    ...
    const data = await response.json() as { data?: SessionEvent[] };
    return data.data ?? [];
  } catch (error) {
...
export class SessionApiClient {
  ...
  constructor(accessToken: string, model: string = 'claude-sonnet-4-20250514') {
 ...
 */
export async function createSessionClient(model?: string): Promise<SessionApiClient | null> {
  const credentials = readCredentials();
