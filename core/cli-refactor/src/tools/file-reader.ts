 ...
 */
export function detectEncoding(filePath: string): FileEncoding {
  try {
 ...
 */
export function readTextFile(options: FileReadOptions): TextFileResult {
  const { filePath, offset = 1, limit } = options;
  const startOffset = offset === 0 ? 0 : offset - 1;

  const { content, lineCount, totalLines } = readFileLines(
    filePath,
    startOffset,
    limit
  );
 ...
 */
export function isFileSafeToRead(
  filePath: string,
 ...
 */
export function tabsToSpaces(content: string, spacesPerTab: number = 2): string {
