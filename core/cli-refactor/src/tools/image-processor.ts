...
import type { ImageFileResult, ImageMimeType } from './types.js';
...
function createImageResult(
  ...
  const mediaType: ImageMimeType = `image/${format === 'jpg' ? 'jpeg' : format}` as ImageMimeType;
...
export async function processImage(
  filePath: string,
  maxTokens: number = DEFAULT_MAX_IMAGE_TOKENS,
  format?: string
  ...
  console.warn(
    `Image at ${filePath} exceeds token limit. ` +
...
export async function processLargeImage(
  ...
  const estimatedTokens = estimateImageTokens(
    Buffer.from(result.file.base64, 'base64').length
  );

  if (estimatedTokens > maxTokens) {
    // In a full implementation, apply more aggressive compression
    console.warn(
      `Large image at ${filePath} may exceed token limits. ` +
