 ...
 * ```
 */
export function extendedEncodeURIComponent(input: string): string {
 ...
 * ```
 */
export function parseRfc3339DateTimeWithOffset(value: string): Date {
  const date = new Date(value);
  if (isNaN(date.getTime())) {
 ...
 * ```
 */
export function formatRfc3339DateTime(date: Date): string {
 ...
 */
export function strictParseInt32(value: unknown): number | undefined {
  if (value === null || value === undefined) {
    return undefined;
  }
 ...
 * ```
 */
export function buildFormUrlencodedString(params: Record<string, string>): string {
...
export async function collectBody(
  ...
  streamCollector?: (stream: ReadableStream<Uint8Array>) => Promise<Uint8Array>
): Promise<Uint8Array> {
