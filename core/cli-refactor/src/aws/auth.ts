...
export class DefaultIdentityProviderConfig implements IdentityProviderConfig {
  private authSchemes: Map<string, IdentityProvider>;
...
export class HttpApiKeyAuthSigner implements HttpSigner {
  ...
  async sign(
    ...
    const { name, in: location, scheme } = signingProperties as {
      name?: string;
      in?: HttpApiKeyAuthLocation;
      scheme?: string;
    };
    ...
    const apiKeyIdentity = identity as ApiKeyIdentity;
    ...
    if (location === HttpApiKeyAuthLocation.QUERY) {
      ...
      throw new Error(
        `request can only be signed with \`apiKey\` locations \`query\` or \`header\`, but found: \`${location}\``
      );
    }
...
export class HttpBearerAuthSigner implements HttpSigner {
  ...
  async sign(
    request: HttpRequest,
    identity: Identity,
    _signingProperties?: Record<string, unknown>
  ): Promise<HttpRequest> {
 ...
 */
export function doesIdentityRequireRefresh(identity: Identity): boolean {
 ...
 */
export function createIsIdentityExpiredFunction(
  thresholdMs: number
): (identity: Identity) => boolean {
  return (identity: Identity): boolean => {
    return (
 ...
 */
export function memoizeIdentityProvider<T extends Identity>(
  identityProvider: IdentityProvider | undefined,
  isExpired?: (identity: T) => boolean,
  requiresRefresh?: (identity: T) => boolean
): ((options?: { forceRefresh?: boolean }) => Promise<T>) | undefined {
  ...
  const provider =
    typeof identityProvider !== 'function'
      ? async () => Promise.resolve(identityProvider as T)
      : identityProvider;

  let memoizedIdentity: T | undefined;
  let pending: Promise<T> | undefined;
  let hasResolved = false;
  let isConstant = false;

  const coalesceProvider = async (options?: { forceRefresh?: boolean }): Promise<T> => {
    if (!pending) {
      pending = provider(options as Record<string, unknown>) as Promise<T>;
    }
  ...
  if (isExpired === undefined) {
    return async (options?: { forceRefresh?: boolean }): Promise<T> => {
 ...
 */
export function normalizeProvider<T>(provider: T | (() => Promise<T>)): () => Promise<T> {
  if (typeof provider === 'function') {
    return provider as () => Promise<T>;
  }
 ...
 */
export function getSmithyContext(context: Record<string, unknown>): Record<string, unknown> {
  return (context[SMITHY_CONTEXT_KEY] as Record<string, unknown>) || (context[SMITHY_CONTEXT_KEY] = {});
}
...
export function setFeature(
  ...
  const smithyContext = context as { __smithy_context?: { features?: Record<string, string> } };
 ...
 */
export function setCredentialFeature<T extends AWSCredentials>(
  credentials: T,
  feature: string,
  code: string
): T {
  const enhanced = credentials as T & { $source?: Record<string, string> };
...
export function resolveAuthOptions(
  ...
  const preferred: HttpAuthScheme[] = [];
